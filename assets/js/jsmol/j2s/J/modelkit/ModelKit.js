...
Clazz.load (["JU.BS", "$.P3", "J.i18n.GT"], "J.modelkit.ModelKit", ["java.lang.Boolean", "$.Double", "$.Float", "$.IllegalArgumentException", "java.util.Arrays", "$.Hashtable", "JU.AU", "$.Lst", "$.Measure", "$.P4", "$.PT", "$.Quat", "$.SB", "$.V3", "JM.Atom", "$.AtomCollection", "$.ModelSet", "JS.SV", "JU.BSUtil", "$.Edge", "$.Elements", "$.Escape", "$.Logger", "$.SimpleUnitCell", "JV.Viewer"], function () {
...
var sym = this.vwr.ms.getUnitCell (mi);
...
if (basis == null) basis = sym.removeDuplicates (this.vwr.ms, bsAtoms);
...
var ok = this.assignBond (bondIndex, type, bsAtoms);
this.vwr.ms.setAtomNamesAndNumbers (a1.i, -ac, null, true);
if (!ok || type == '0') this.vwr.refresh (3, "setBondOrder");
...
Clazz.defineMethod (c$, "assignBond", 
 function (bondIndex, type, bsAtoms) {
...
var atoms = this.vwr.ms.at;
if (index < 0 || index2 < 0 || index >= atoms.length || index2 >= atoms.length || atoms[index] == null || atoms[index2] == null) return;
var state = this.getMKState ();
try {
var connections = JU.AU.newFloat2 (1);
connections[0] =  Clazz.newFloatArray (-1, [index, index2]);
var modelIndex = atoms[index].mi;
var bs = JU.BSUtil.newAndSetBit (index);
bs.set (index2);
var bsBonds = this.vwr.ms.getBondsForSelectedAtoms (bs, false);
...
function (bsSelected, iatom, p, allowProjection) {
...
if (bsAU.get (bai)) {
continue;
}if (this.getConstraint (sg, bai, J.modelkit.ModelKit.GET_CREATE).type == 6) {
return 0;
}bsAU.set (bai);
bsAtoms2.set (i);
}
...
m.rotate (vt);
p.add2 (center, vt);
this.getConstraint (sg, i, J.modelkit.ModelKit.GET_CREATE).constrain (a, p, false);
