...
class Elementoj {
    ...
    async ŝargu() {
        const resp = await fetch(e_tabelo);
        this.pselementoj = JSON.parse(await resp.text());
        //console.log(this.pselementoj[0]);
    }
    ...
    static #simboloj = Object.fromEntries(Elementoj.#elementoj.map(
        (e,n) => [e[0],n+1])
    );

    /**
     * Redonas perilo-objekton por ebligi aliri la elementojn per <perilo>[2] kaj <perilo>.He
     * anstataŭ per Elementoj.nro(2) kaj Elementoj.smb('He')
     */
    static listo() {
        const Elemento_perilo = {
            get: function(target, prop, receiver) {
                if (! isNaN(prop) ) { // (typeof prop === 'number') {
                    return Elementoj.nro(prop)
                } else if (typeof prop === 'string') {
                    return Elementoj.smb(prop)
                } else {
    ...
    */
    static nro(n) {
        const e = Elementoj.#elementoj[n-1];
        return Object.assign({nro: n},Elementoj._obj(e));
    }
     ...
     * 
     */
    periodsistemo(svg,indikoj,kiam_navigo) {
        ...
        function erekt(elm) {    
            ...
            let e_detale;
            if (indikoj & Elementoj.AMAS || indikoj & Elementoj.DENS) {
                e_detale = self.pselementoj[elm.nro-1]//json_elementoj[elm.nro]
            }   

            // atommaso
            let ap = '';
            if (indikoj & Elementoj.AMAS) {
                ap = document.createElementNS(ns,"text");
                ap.textContent = isNaN(e_detale.maso)? ''
                    : parseFloat(e_detale.maso).toFixed(2).replace('.',',');
                atributoj(ap,{
                    x: 9.5,
                    y: 9.5,
                    class: "amas"
            ...
            // atomdenso
            let ad = '';
            if (indikoj & Elementoj.DENS) {
                ad = document.createElementNS(ns,"text");
                ad.textContent = isNaN(e_detale.denso)? '':e_detale.denso;
                atributoj(ad,{
                    x: 9.5,
                    y: 9.5,
                    class: "dens"
        ...
        if (kiam_navigo) { 
            ...
            if (event.keyCode >= 37 && event.keyCode <= 40) {
                ...
                if (ps_emfazita_elemento) {
                    const elm = Elementoj.smb(ps_emfazita_elemento);    
                    ne = nav(elm.nro,event.keyCode); // numero de la nova elemento
                }

                const nova = Elementoj.nro(ne);
    ...
    /**
     * Redonas elementon el la Json-tabelo laŭ simbolo
    ...
    static e_distribuo(smb,sen_ng) {
        const elm = Elementoj.json_elemento(smb);
    ...
    static e_distr(smb,ŝ,sŝ,ne) {
        ...
        const a_ss = [
            ...
            ['6p','7p']];
            */
        const distr = Elementoj.e_distribuo(smb,true);
